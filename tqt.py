...
def get_slice(input, slices, delim):
    ...
    #print >> sys.stderr, "get_slice... input.split(%s) = %s" % (delim, input.split(delim))
    output = []
    for slice in slices:
        try:
            output.append(input.split(delim)[slice])
        except IndexError:
            print >> sys.stderr, "unindexable line, probably end of file"
...
def redux(agg,opts):
    ...
    if opts.action == "mean":
        return reduce(lambda x, y: float(x) + float(y), agg)/len(agg)
...
def engage(opts, args):
    ...
    if opts.sorted:
        ...
        for line in infile.readlines():
            #print >> sys.stderr, "XXXXXX key=", key
            line = line.strip()
        ...
        for line in infile.readlines():
            ...
            newkey = get_slice(line, keyslices, opts.in_delim)
            newvalue = get_slice(line, valueslices, opts.in_delim)
            #print >> sys.stderr, "newkey=", newkey, "newvalue=", newvalue
            if newkey:
...
if __name__ == "__main__":
    parser = OptionParser()
    parser.add_option("-a", "--action", default="concat", action="store", choices=["concat", "add", "mean", "median", "stats"], help="method used to combine/aggregate the data, options beyond concat assume a single field is being aggregated and that said field can be parse as a digit")
